  ...
  while cap.isOpened():
    ...
    if results.multi_face_landmarks:
      for face_landmarks in results.multi_face_landmarks:
        ...
        roll = math.atan2(-1 * (nose_tip[1] - (left_eye[1] + right_eye[1])/2), (nose_tip[0] - (left_eye[0] + right_eye[0])/2))

        # Draw face pose information
        cv2.putText(image, f"Distance: {distance:.2f} cm", (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.putText(image, f"Yaw: {yaw*180/math.pi:.2f} degrees", (50, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.putText(image, f"Pitch: {pitch*180/math.pi:.2f} degrees", (50, 300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.putText(image, f"Roll:{roll*180/math.pi:.2f} degrees", (50, 350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
